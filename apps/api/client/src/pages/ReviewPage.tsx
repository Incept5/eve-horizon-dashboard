...
const mockReviewJobs: ReviewJob[] = [
  {
    id: '1',
    subject: 'Add user authentication',
    description: 'Implements login/logout flow with JWT tokens and refresh token rotation. Includes middleware for protected routes and session management.',
    phase: 'review',
    status: 'pending_review',
    submitted_at: '2026-01-29T08:00:00Z',
  ...
  },
  {
    ...
    description: 'Resolves timeout issues under load by implementing proper connection pool sizing and retry logic with exponential backoff.',
    phase: 'review',
    status: 'pending_review',
    submitted_at: '2026-01-29T07:30:00Z',
...
export function ReviewPage() {
  ...
  const [selectedJobForReject, setSelectedJobForReject] = useState<ReviewJob | null>(null);

  const filteredJobs = activeTab === 'needs-my-review'
    ? jobs.filter(job => job.status === 'pending_review')
    : jobs;

  const getTimeInReview = (submittedAt: string): string => {
    const now = new Date();
  ...
  const handleConfirmReject = (reason: string) => {
    if (selectedJobForReject) {
      setIsLoading(true);
      // Simulate API call with rejection reason
      console.log(`Rejecting job ${selectedJobForReject.id} with reason: ${reason}`);
      setTimeout(() => {
        setJobs(jobs.filter(job => job.id !== selectedJobForReject.id));
        setIsLoading(false);
              ...
              <div className="space-y-4">
                ...
                <div className="flex items-center gap-6 text-sm text-eve-400">
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    <span>{getTimeInReview(job.submitted_at)} in review</span>
                  </div>
                  ...
                  {job.pr_url && (
                    <a
                      ...
                      className="flex items-center gap-2 text-eve-300 hover:text-eve-200 transition-colors"
